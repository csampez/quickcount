...
remesa_path<-"Q://"
#remesa_path<-sprintf('remesas/')

#output_path<-sprintf("%s/buzon_senadores/nieto/",path)
output_path<-"W://"
#output_path<-"./estimaciones/"

output_path_escano<-"X://"
#output_path_escano<-sprintf("%s/buzon_senadores_escano/nietosen/",path)
#output_path_escano<-"./estimaciones/"

get_estimates_dip<-function(remesa_name, remesa_path, output_path, output_path_escano){
  
  print(sprintf("Inicia estimacion para remesa: %s", remesa_name))
  time<-now()
  
  #id = identificador de remesa
  # remesa_name<-"051740"
  id<-sprintf("0400%s",remesa_name)
  ...
  print(sprintf("Remesa %s",remesa_name))
  ...
  czeros<-remesa %>% filter(TOTAL==0) %>% dim
  # tryCatch(
  #   { 
  ...
  if(c1!=c0) {
    ...
    cat(paste("Numero de casillas con cero votos en remesa =",czeros[1]),"\n")  
  }
  ...
  if(c1>0){
    ...
    sim_est<-function(id_est){
      #id_est<-1
      dfe<-map(c(partido,"NOVOTO", "NOVOTO_ADJ"), function(x){sim_partido(id_est, x)}) %>% bind_rows()
    ...
    # clusterEvalQ(cl, c(library(truncnorm), library(tidyverse)))
    # df_all<-parLapply(cl,strata,sim_est) %>% bind_rows()
    # stopCluster(cl)
    # 
    df_all<-map(strata, sim_est) %>% bind_rows()
    ...
    #print(cot.c %>% filter(LMU==1) %>% select("PAN","PRI","PRD","PVEM","PT","MC","MORENA") %>% sum)
    ...
    write.csv(cot.c,
              file=sprintf("%s/nietodip40%s.csv", output_path_escano,remesa_name),
              row.names=FALSE,quote=FALSE)
    print("Escribe diputados")
    
    print(sprintf("Tiempo de ejecucion:%s", as.character(now()-time)))
...
# },
# error = function(cond) {
#   message(paste("Hay algo raro con la remesa", remesa_name))},
# finally = message(paste("Hay algo raro con la remesa:", remesa_name))
# )
# }
...
while(TRUE){
  estimaciones_list<-dir(sprintf("%s/", output_path)) %>% map(function(x){substr(x,8,13)}) %>% unlist 
  rem_list <- dir(sprintf("%s/", remesa_path))
  remesas_list<- rem_list[rem_list %>% str_detect(pattern = "REMESAS")] %>% map(function(x){substr(x,12,17)}) %>% unlist
  remesas_list<- remesas_list[!remesas_list %in% c("021645", "021650", "021655", "021700","021705","021710","021715","021720","021725","021730",
                                                   "021735", "021740", "021745", "021750", "021755", "021800", "021805", "021810", "021815", "021830")]
  ...
  #     mclapply(.,function(x){get_estimates_dip(x, remesa_path, output_path, output_path_escano)}, mc.cores=num_cores)
  # print(now()-time)
  
  setdiff(remesas_list, estimaciones_list) %>%
    sort(decreasing = TRUE) %>%
    map(.,function(x){ get_estimates_dip(x, remesa_path, output_path, output_path_escano)})
  Sys.sleep(15)
}

#get_estimates_dip("051700", remesa_path, output_path, output_path_escano)
