...
output_path_escano<-"O://"


#remesa_path<-sprintf('../../../senadores')
...
#get_estimates_sen(remesa_name, remesa_path, output_path, output_path_escano)

get_estimates_sen<-function(remesa_name, remesa_path, output_path, output_path_escano){
  ...
  if(c1>0){
    ...
    print(cot.d %>% filter(LMU==1) %>% select("PAN","PRI","PRD","PVEM","PT","MC","MORENA") %>% sum)
    
    write.csv(cot.d %>% 
                select(all_of(orden_cotecora)),
              
              file=sprintf("%s/nietosen50%s.csv",output_path_escano, remesa_name), 
              row.names=FALSE, 
              quote=FALSE)
    
    print("Escribe senadores")
...
#num_cores<-detectCores()-2
#time<-now()
#get_estimates_sen("051650", remesa_path, output_path, output_path_escano)
#now()-time
